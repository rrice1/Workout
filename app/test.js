...
ok("Madcow Friday: top triple (PR) then a back-off set of 8", (() => { const s = G.buildMadcowSession(DATA, { day: "Madcow — Friday (Intensity)", today, maxes: pctMax, settings: pctBars }).blocks[0].items[0].sets; return s.length === 6 && s[4].reps === 3 && s[4].him === Math.max(...s.map(x => x.him)) && s[5].reps === 8 && s[5].him < s[4].him; })());
ok("Madcow Wednesday OHP is a 4-set ramp", G.buildMadcowSession(DATA, { day: "Madcow — Wednesday (Light)", today, maxes: pctMax, settings: pctBars }).blocks[0].items.find(i => i.base === "press").sets.length === 4);
